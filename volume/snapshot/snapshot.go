...
type Interface interface {
	...
	Delete() (*kahuapi.Snapshot, error)
	GetSnapshots() ([]*kahuapi.Snapshot, error)
}
...
func newSnapshot(kubeClient kubernetes.Interface,
	kahuClient clientset.Interface,
...
func (s *snapshoter) Apply() error {
	...
	byProvisioner := make(map[string][]kahuapi.ResourceObjectReference)
	for _, volume := range volumes {
		...
		if !ok {
			byProvisioner[provisioner] = make([]kahuapi.ResourceObjectReference, 0)
		}
...
func (s *snapshoter) getPV(volume kahuapi.ResourceObjectReference) (*corev1.PersistentVolume, error) {
	switch volume.Kind {
	case utils.PVC:
...
func (s *snapshoter) kahuSnapshot(backup string,
	provisioner string, volumes []kahuapi.ResourceObjectReference) (*kahuapi.Snapshot, error) {
	kahuSnapshot := s.volGroupToSnapshot(backup, provisioner, volumes)

	return s.kahuClient.KahuV1beta1().Snapshots().Create(context.TODO(), kahuSnapshot, metav1.CreateOptions{})
}

func (s *snapshoter) volGroupToSnapshot(backup string,
	provisioner string,
	vols []kahuapi.ResourceObjectReference) *kahuapi.Snapshot {
	return &kahuapi.Snapshot{
...
func (s *snapshoter) Delete() (*kahuapi.Snapshot, error) {
	return nil, nil
}
...
func snapshotReadyToUse(snapshot *kahuapi.Snapshot) bool {
...
func (s *snapshoter) GetSnapshots() ([]*kahuapi.Snapshot, error) {
	kahuVolSnapshots := make([]*kahuapi.Snapshot, 0)
