...
func (f *factory) BySnapshot(s snapshot.Interface) (Interface, error) {
	...
	snapshotRefs := make([]kahuapi.ResourceObjectReference, 0)
	for _, kahuVolSnapshot := range kahuVolSnapshots {
		snapshotRefs = append(snapshotRefs, kahuapi.ResourceObjectReference{
...
func (f *factory) ByVolumes(vg volumegroup.Interface) (Interface, error) {
	...
	volRefs := make([]kahuapi.ResourceObjectReference, 0)
	for _, vol := range volumes {
		volRefs = append(volRefs, kahuapi.ResourceObjectReference{
			Kind:      utils.PVC,
			Name:      vol.Name,
