...
type ResourceHook struct {
	Name         string
	Pre          []kahuv1.ResourceHook
	Post         []kahuv1.ResourceHook
	namespacesIn []string
	namespacesEx []string
	resourcesIn  []kahuv1.ResourceIncluder
	resourcesEx  []kahuv1.ResourceIncluder
...
func NewHooks(config *rest.Config, hookSpecs []kahuv1.ResourceHookSpec) (*Hooks, error) {
...
func getHooksSpec(hookSpecs []kahuv1.ResourceHookSpec) ([]ResourceHook, error) {
...
func (h *Hooks) ExecuteHook(
	...
	if hookExec != nil {
		...
		if err != nil {
			...
			if hookExec.OnError == kahuv1.HookErrorModeFail {
				return err
			}
		}
		// Skip normal hooks
	...
	for _, resourceHook := range h.resourceHooks {
		...
		for _, hook := range hooks {
			if hook.Exec != nil {
				...
				if err != nil {
					if hook.Exec.OnError == kahuv1.HookErrorModeFail {
						return err
					}
				}
			}
...
func getHooksSpecFromAnnotations(annotations map[string]string, stage string) *kahuv1.ExecHook {
	...
	execSpec := kahuv1.ExecHook{
		Command:   command,
		Container: container,
		OnError:   kahuv1.HookErrorMode(onError),
...
func (h *Hooks) executePodCommand(pod *v1.Pod, namespace, name, hookName string, hook *kahuv1.ExecHook) error {
	if pod == nil {
	...
	case kahuv1.HookErrorModeFail, kahuv1.HookErrorModeContinue:
		// use the specified value
	default:
		// default to fail
		localHook.OnError = kahuv1.HookErrorModeFail
	}
...
func setDefaultHookContainer(pod *v1.Pod, hook *kahuv1.ExecHook) error {
