...
func (ctrl *controller) getSnapshotBySnapshotState(
	state kahuapi.SnapshotState) (*providerSvc.Snapshot, error) {
	csiSnapsotRef := state.CSISnapshotRef

	ctrl.logger.Errorf("csiSnapsotRef : %v", csiSnapsotRef)

	csiVolSnapshot, err := ctrl.csiSnapshot.VolumeSnapshots(csiSnapsotRef.Namespace).
		Get(context.TODO(), csiSnapsotRef.Name, metav1.GetOptions{})
	if err != nil {
		return nil, err
	}
...
func (ctrl *controller) getPVBySnapshotState(
	state kahuapi.SnapshotState) (*v1.PersistentVolume, error) {
