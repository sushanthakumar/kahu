...
state towards the desired state`,
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			// get provider info
			identityClient := pb.NewIdentityClient(grpcConn)
			providerInfo, err := identityClient.GetProviderInfo(context.Background(),
				&pb.GetProviderInfoRequest{})
			if err != nil {
			...
			err = providerIdentity.RegisterVolumeProvider(ctx, &grpcConn)
			if err != nil {
...
func Run(ctx context.Context,
	...
	grpcConn grpc.ClientConnInterface) error {
	log.Info("Starting volume service")
	log.Info("Build info ")
	utils.GetBuildInfo().Print()
	log.Info("Configuration")
	config.Print()

	backupProviderClient := pb.NewVolumeBackupClient(grpcConn)
	...
	cacheSyncResults := config.InformerFactory.WaitForCacheSync(ctx.Done())
	...
	log.Info("Starting volume backup service")
